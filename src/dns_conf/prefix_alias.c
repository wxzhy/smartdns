...
int _conf_prefix_alias(const char *ip_cidr, const char *ips, int prefix_len)
{
	...
	if (strncmp(ips, "ip-set:", sizeof("ip-set:") - 1) == 0) {
		if (_config_ip_rule_set_each(ips + sizeof("ip-set:") - 1, _config_prefix_alias_add_ip_callback, prefix_alias) != 0) {
			tlog(TLOG_ERROR, "Failed to process ip-set for prefix-alias");
			goto errout;
		}
	} else {
		target_ips = strdup(ips);
		if (target_ips == NULL) {
			tlog(TLOG_ERROR, "Failed to allocate memory for target_ips");
			goto errout;
		}
		...
		for (char *tok = strtok(target_ips, ","); tok != NULL; tok = strtok(NULL, ",")) {
			...
			if (ret != 0) {
				tlog(TLOG_ERROR, "Failed to add prefix-alias IP: %s", tok);
				goto errout;
			}
		}
	}
	...
	if (_config_ip_rule_add(ip_cidr, IP_RULE_PREFIX_ALIAS, prefix_alias) != 0) {
		tlog(TLOG_ERROR, "Failed to add prefix-alias rule for %s", ip_cidr);
		goto errout;
	}
...
int _config_prefix_alias(void *data, int argc, char *argv[])
{
	...
	if (ret != 0) {
		tlog(TLOG_ERROR, "prefix-alias: failed to configure prefix alias");
		return -1;
	}
